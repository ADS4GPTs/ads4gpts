# This is a template for a langchain agent prompt. The agent is responsible for providing the ad toolkit with all necessary, accurate, and
# well-structured data to enable it to select the most appropriate and impactful advertisements. The agent's work is characterized by analytical precision, ethical adherence, and a
# commitment to maximizing ad relevance and user engagement.

# The Ad prompt is meant for the instructions that you want to give to the agent to call the ADS4GPTs tool to generate an ad. It could be a simple "Retrieve One Chat Ad" or "Retrieve One Banner Ad" or
# any other instructions that you want to give to the agent. The agent uses as context the conversation with the user and the prompt that you provide to generate the ad.
